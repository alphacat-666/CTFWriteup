...
After extracting, we get 248 new images, which only showing 1-248 numbers. There is nothing special from the first glance.
...
But among these 248 images, some are displayed normally, some are reporting errors: `Fatal error reading PNG image file: invalid IHDR data`.

So I decide to make a comparison between normal one and abnormal one.
...
We can extract each bit of this position, so that we get a 248-bit binary data. For every 8 bits, convert them into one ASCII character.

```python
import os

folder = "./_logo.png.extracted"
bits = ""
...
```
01110100011010100110001101110100011001100111101101110111011011110111011101011111011110010110111101110101010111110110000101100011011101000111010101100001011011000110110001111001010111110111001001100101011000010110010001011111011010010111010001111101
